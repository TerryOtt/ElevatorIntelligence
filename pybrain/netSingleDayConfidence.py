...
def writeNetResults(neuralNet, simDate, csvFilename):
    ...
    simStep = datetime.timedelta(hours=1)


    while simTime < simEndTime:
             
        simTimestamp = simTime.strftime("%Y%m%d %H%M%S")
        neuralNetResultArray = activateNet(neuralNet, simTimestamp)

        print( "\nTime {0}".format(simTime.strftime("%H:%M:%S")) )

        pprint.pprint(neuralNetResultArray)
...
def activateNet(neuralNet, entryTimestamp):
    ...
    return neuralNet.activate( inputVector )
